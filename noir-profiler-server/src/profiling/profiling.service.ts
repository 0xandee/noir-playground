...
import { ProfilingResult, SvgResult, CircuitMetrics, CircuitFunctionMetrics } from './interfaces/profiling-result.interface';
...
export class ProfilingService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.backendPath = this.configService.getOrThrow<string>('noir.backendPath');
  }
  ...
  async profileOpcodes(dto: ProfileOpcodesDto): Promise<ProfilingResult> {
    ...
    try {
      ...
      this.logger.log(`Created temporary files: artifact=${artifactPath}, source=${sourceCodePath}${cargoTomlPath ? `, cargo=${cargoTomlPath}` : ''}`);
      ...
      for (const svgFile of svgFiles) {
        ...
        const functionMatch = svgFile.match(/^(.+?)_(?:acir_opcodes|brillig_opcodes|gates)\.svg$/);
  ...
  async checkProfilerAvailability(): Promise<{ available: boolean; version?: string; message: string }> {
    try {
      ...
      return {
        available: false,
        message: 'Noir profiler is not available. Make sure it\'s installed and in PATH.'
      };
    }
  }

  private async cleanupRequestDir(
